#!/bin/bash
# skill-manager: PostToolUseFailure hook (fires on FAILURE only)
...
log_debug "post-tool-failure: skill=${SKILL_NAME} success=false duration=${DURATION_MS}ms"

append_to_chain "$SESSION_ID" "$SKILL_NAME" "false" "$DURATION_MS" "0"
update_weekly "$WEEK" "$SKILL_NAME" "false" "$DURATION_MS" "0"
